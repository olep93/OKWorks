...
5. Webhook-mottak med signaturkontroll, idempotent hendelseslogg og statuser for levert, forsinket, avvist, spamrapportert, feilet og blokkert er implementert. Opprett webhooken i Resend for `https://okfaktura.no/api/email/resend/webhook`, velg e-posthendelsene og lagre signing secret som `RESEND_WEBHOOK_SECRET` i Vercel Production før funksjonen aktiveres.
