...
const schema = z.object({ origin: z.string().trim().min(3).max(500), destination: z.string().trim().min(3).max(500), emissionType: z.enum(["GASOLINE", "DIESEL", "HYBRID", "ELECTRIC"]).optional(), date: z.string().refine(validDate).optional(), time: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/).default("08:00"), autoPass: z.boolean().optional(), returnTrip: z.object({ date: z.string().refine(validDate), time: z.string().regex(/^([01]\d|2[0-3]):[0-5]\d$/) }).optional() });
...
export async function GET(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    return NextResponse.json({ origin: stay?.hotelAddress || fullAddress(data.org ?? {}), destination: data.row.order.workAddress || fullAddress(data.row.customer), hotelStay: stay, mileageRateOre: data.settings?.mileageRateOre ?? 500, vehicleName: data.settings?.vehicleName, emissionType: data.settings?.vehicleFuelType || "GASOLINE", autoPass: data.settings?.vehicleAutoPass ?? false, mapsConfigured: Boolean(process.env.GOOGLE_MAPS_API_KEY || process.env.DIB_TOLL_API_KEY), tollProvider: process.env.DIB_TOLL_API_KEY ? "DIB" : "GOOGLE" });
...
export async function POST(request: Request, ctx: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (parsed.data.returnTrip && parsed.data.returnTrip.date < (parsed.data.date || localDate())) return NextResponse.json({ error: "Returdato kan ikke være før utreisedato." }, { status: 400 });
    return NextResponse.json(await computeTravelRoutes(parsed.data.origin, parsed.data.destination, parsed.data.emissionType || data.settings?.vehicleFuelType || "GASOLINE", parsed.data.date || localDate(), parsed.data.time, parsed.data.autoPass ?? data.settings?.vehicleAutoPass ?? false, parsed.data.returnTrip));
